...
var H5ComponentPie =function ( name, cfg ) {
    ...
    var draw = function( per ){
        ...
        if( per >= 1){
            ...
            component.find('.text').css('-webkit-transition','all 0s');
            H5ComponentPie.reSort(component.find('.text'));
            component.find('.text').css('transition','all 1s');
            component.find('.text').css('-webkit-transition','all 1s');
...
H5ComponentPie.reSort  = function ( list ) {
    ...
    $.each(list,function (i,domTaget) {
        if (compare(willReset[willReset.length - 1],domTaget)) {
            willReset.push(domTaget);//不会把自身加入到对比
        }
    });
